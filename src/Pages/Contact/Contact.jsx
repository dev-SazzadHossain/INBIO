...
const Contact = () => {
    ...
    const handelSubmit=e=>{
        ...
        const templateParams = {
         ...
         if([nameValue,emailValue,subjectValue,messageValue,phoneValue].some(filed=>filed == "")){
            Swal.fire({
                position: "top-end",
                icon: "warning",
...
<div className=' w-full lg:h-[600px]  md:h-[630px] h-[600px] shadow-md p-5 rounded-md shadow-violet-500/50 group/pic lg:mb-0 md:mb-0  mb-5'>
    ...
    <ul>
     ...
     <div  className=' w-full flex flex-col justify-center items-center lg:items-start md:items-start'>
              ...
              <Link to="https://www.facebook.com/profile.php?id=100011271842054" target='_blank'>  <div className=' w-[50px] h-[50px] bg-gradient-to-r from-violet-500 to-pink-500 rounded-lg text-white flex items-center justify-center group cursor-pointer transition-all duration-300 shadow-lg shadow-violet-500/55 hover:shadow-red-500/55'>
                ...
                <Link to="https://www.linkedin.com/in/md-sazzad-hossain-b11a31254/" target='_blank'>    <div className=' w-[50px] h-[50px] bg-gradient-to-r from-violet-500 to-pink-500 rounded-lg text-white flex items-center justify-center group cursor-pointer transition-all duration-300 shadow-lg shadow-violet-500/55 hover:shadow-red-500/55'>
...
<form ref={form}  className='w-full' onSubmit={handelSubmit}>
  ...
  <div className='w-full'>
       ...
       <div> <input onChange={(e)=>setName(e.target.value)} className='w-full border-none outline-none shadow-md px-2 py-3 text-lg tracking-tight font-medium shadow-violet-500/55' type="text" name="name" placeholder='Your Name' id="" /></div>
    </div>
    <div className='w-full'>
        <label className='text-lg font-bold py-2 inline-block text-gray-500' htmlFor="name" >Phone Number</label>
       <div> <input onChange={(e)=>setPhoneNumber(e.target.value)} className='w-full border-none outline-none shadow-md px-2 py-3 text-lg tracking-tight font-medium shadow-violet-500/55' type="number"  name="phoneNumber" placeholder='Phone Number' id="" /></div>
    </div>
  </div>
    <div className='pt-3'>
        <label className='text-lg font-bold py-2 inline-block text-gray-500' htmlFor="name">Email</label>
       <div> <input onChange={(e)=>setEmail(e.target.value)} className='w-full border-none outline-none shadow-md px-2 py-3 text-lg tracking-tight font-medium shadow-violet-500/55' type="email" name="email" placeholder='Email' id="" /></div>
    </div>
    <div className='pt-3'>
        <label className='text-lg font-bold py-2 inline-block text-gray-500' htmlFor="name">Subject</label>
       <div> <input onChange={(e)=>setSubject(e.target.value)} className='w-full border-none outline-none shadow-md px-2 py-3 text-lg tracking-tight font-medium shadow-violet-500/55' type="text" name="subject" placeholder='Subject' id="" /></div>
    </div>
    <div className='pt-3'>
        <label className='text-lg font-bold py-2 inline-block text-gray-500' htmlFor="name">Your Message</label>
       <div> <textarea onChange={(e)=>setMessage(e.target.value)} row="4" className='w-full border-none outline-none shadow-md px-2 py-3 text-lg tracking-tight font-medium shadow-violet-500/55' type="text" name="message" placeholder='Your Message' id="" /></div>
    </div>

    <div className='pt-8 group'>
        <button type='submit' className=' w-full text-center bg-gradient-to-r from-violet-500 to-pink-500 py-3 border-none outline-none rounded-lg text-white font-semibold tracking-wide flex justify-center items-center gap-5 group-hover:shadow-md group-hover:shadow-gray-600/55 duration-300 transition-all'>{loading ? "SENDING..":"SEND MESSAGE"} <span className='group-hover:pl-2 duration-300 transition-all'><FaArrowRight/></span></button>
    </div>
</form>
